...
export const externalCheckin = {
  prompts: [
    ...
    { id: 'site.analysis', prompt: 'The report requires both supported documentation versions and the AI Ready database. Use existing documentation routes to confirm rendering after a collection failure. Check Pages deployment and content synchronization before changing documentation sources. Keep local previews separate from production evidence. Use the installed Sentry check-in triage skill for issue details and verified repair proposals. Inspect release, URL, browser, affected users, and recurrence before classifying issues. Expected stale-documentation 404s and local preview errors follow existing Sentry policy. Do not resolve issues without a verified deployed repair. No system-health email exists. Do not create one or send test messages. Never deploy, mutate production data, or send messages during check-in.' },
  ],
