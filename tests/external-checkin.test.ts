...
it('validates report deployment and keeps missing Sentry coverage visible', async () => {
  ...
  const request = (async (_url, options) => {
    ...
    return Response.json({
      ...
      identity: { site: 'unhead.unjs.io', environment: 'production', deployment: 'old' },
      results: ['unhead.docs-v3', 'unhead.docs-v2', 'unhead.ai-ready'].map(id => ({ id, result: { _tag: 'Pass', evidence: {} } })),
    })
  }) as typeof fetch
