...
const identity = { site: 'unhead.unjs.io', environment: 'production', deployment: 'release-1' }
...
function siteReport(deployment = identity.deployment, observedAt = now) {
  return runChecks(['unhead.docs-v3', 'unhead.docs-v2', 'unhead.ai-ready'].map(id => defineCheck({ id, run: () => pass({}) })), { now: observedAt, identity: { ...identity, deployment } })
}
