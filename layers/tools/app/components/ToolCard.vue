  ...
  <section class="px-3 sm:px-6 lg:px-8 pb-12">
    <div class="mx-auto" :class="[maxWidth]">
      <div class="relative">
        ...
        <div class="relative bg-default rounded-xl sm:rounded-2xl overflow-hidden shadow-xl ring-1 ring-[var(--ui-border)]">
          <div class="flex items-center gap-2 px-4 sm:px-6 py-3 border-b border-default">
            <UIcon :name="icon" class="w-4 h-4" :class="toolIconClass(color)" />
            <span class="text-sm font-semibold">{{ title }}</span>
            <span v-if="subtitle" class="ml-auto text-xs text-dimmed hidden sm:inline">{{ subtitle }}</span>
          </div>
