...
// Derive from route directly instead of calling useCurrentDocPage() redundantly
const isV2 = computed(() => route.path.startsWith('/docs/v2'))
</script>
...
<template>
  ...
  <UMain class="relative mb-20 px-5">
    ...
    <div class="max-w-[1452px] mx-auto lg:pt-5">
      <UPage :ui="{ left: 'lg:col-span-3 xl:col-span-2', center: 'col-span-5 lg:col-span-7 xl:col-span-8' }">
        <template #left>
          <UPageAside :ui="{ root: 'hidden overflow-y-auto lg:block lg:sticky lg:top-(--ui-header-height) pt-2 pb-8 lg:ps-2 lg:-ms-4 lg:pe-3' }" class="max-w-[300px] lg:max-h-[calc(100vh-150px)]">
            <DocsSidebarHeader />
          </UPageAside>
        </template>
