...
function thumbs(thumbs: 'up' | 'down') {
  ...
  $fetch<ThumbsFeedbackResponse>('/api/feedback-thumbs', {
    method: 'POST',
    body: { thumbs, toolId: props.toolId },
  })
    .then(thumbsNextStep)
    .catch((error) => {
...
async function onSubmit(event: FormSubmitEvent<CommentFeedbackSchemaOutput>) {
  ...
  $fetch('/api/feedback', {
    method: 'POST',
    body: JSON.stringify({ ...event.data, toolId: props.toolId }),
  })
    .then(() => {
...
<template>
  <UCard v-if="!hidden" class="mt-6 relative">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      ...
      <div class="flex items-center gap-3">
        ...
        <template v-else-if="thumbSubmissionStatus === 'submitted'">
          <div v-if="thumbsResponse?.thumbs === 'up'" class="flex items-center gap-2 text-green-500">
    ...
    <!-- Comment Form (shown after thumbs down) -->
