...
const unheadGuides = nav.value.filter(c => c.path.startsWith('/docs/guides')).map(enhanceTitlesAndIcons).map(c => ({ ...c, title: 'Unhead', icon: Unhead.icon }))
const schemaOrgAndScriptGuides = nav.value.filter(c => c.path.startsWith('/docs/schema-org') || c.path.startsWith('/docs/scripts')).map(enhanceTitlesAndIcons).map(c => ({
  ...c,
...
<template>
  <footer class="relative z-10 antialiased font-sans dark:bg-[#131822] bg-neutral-50/50 text-sm text-gray-700 dark:text-gray-200">
    ...
    <div class="border-t border-gray-200 dark:border-gray-800">
      <UContainer class="container py-10">
        <h2 class="text-2xl mb-7 font-bold flex items-center gap-1">
          <UIcon dynamic name="i-carbon-notebook" class="w-7 h-7" />
          Guides
        </h2>
        <div class="grid xl:grid-cols-3 lg:gap-20 gap-10">
